...
function getToolLabel(toolName: string, args: Record<string, string>): string {
  const file = args?.path?.split("/").pop() ?? args?.path;
  const newFile = args?.new_path?.split("/").pop() ?? args?.new_path;
  ...
  if (toolName === "str_replace_editor") {
    ...
    if (command === "undo_edit") return `Undoing edit on ${file}`;
  }
  if (toolName === "file_manager") {
    if (command === "delete") return `Deleting ${file}`;
    if (command === "rename") return `Renaming ${file}${newFile ? ` → ${newFile}` : ""}`;
  }
  return toolName;
}
